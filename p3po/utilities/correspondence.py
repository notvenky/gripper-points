...
class Correspondence():
    ...
    def __init__(self, device, dift_path, width, height, image_size_multiplier, ensemble_size, dift_layer, dift_steps):
        """
    ...
    def set_expert_correspondence(self, expert_image):
        with torch.no_grad():
            ...
            expert_img_features = self.dift.forward(expert_image, prompt=self.prompt, ensemble_size=self.ensemble_size, up_ft_index=self.dift_layer, t=self.dift_steps)
    ...
    def find_correspondence(self, expert_img_features, current_image, coords):
        ...
        with torch.no_grad():
            ...
            src_ft = torch.nn.Upsample(size=(self.height, self.width), mode='bilinear', align_corners=True)(src_ft)

            out_coords = torch.zeros(coords.shape)
            for idx, coord in enumerate(coords):
                x, y = int(coord[1] * self.width/self.original_size[0]), int(coord[2] * self.height/self.original_size[1])

                src_vec = src_ft[0, :, y, x].view(1, num_channel).clone() 
                trg_ft = torch.nn.Upsample(size=(self.height, self.width), mode='bilinear', align_corners=True)(ft[1:])
