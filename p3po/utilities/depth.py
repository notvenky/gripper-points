...
class Depth:
    def __init__(self, depth_path, device):
        ...
        dataset = 'hypersim'
        max_depth = 20
        self.depth = DepthAnythingV2(**{**model_configs[encoder], 'max_depth': max_depth})
        self.depth.load_state_dict(torch.load(f'{depth_path}/checkpoints/depth_anything_v2_metric_{dataset}_{encoder}.pth', map_location='cpu', weights_only=False))
