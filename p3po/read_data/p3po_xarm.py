...
class BCDataset(IterableDataset):
    ...
    def _sample(self):
        ...
        if self._prompt == "text":
            ...
            for key in self.keys:
                prompt_pixel = self.aug(prompt_observations[key][-1])[None]
    ...
    def sample_test(self, env_idx, step=None):
        ...
        if self._prompt == "text":
            ...
            for key in self._keys:
                if self._obs_type == "pixels":
                    prompt_input = np.transpose(observations[key][-1:], (0, 3, 1, 2))
                else:
                    prompt_input = observations[key][-1:]
                return_dict["prompt_" + key] = prompt_input
            prompt_proprioceptive_state = np.concatenate(
                [
            ...
            goal_idx = min(
                ...
                len(observations[self._keys[0]]) - 1,
            )
            for key in self._keys:
                if self._obs_type == "pixels":
                    prompt_input = np.transpose(
                        observations[key][goal_idx : goal_idx + 1], (0, 3, 1, 2)
                    )
                else:
                    prompt_input = observations[key][goal_idx : goal_idx + 1]
                return_dict["prompt_" + key] = prompt_input
            prompt_proprioceptive_state = np.concatenate(
                [
